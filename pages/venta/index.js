...
const Venta = () => {
    ...
    const getValorPuntos = async () => {
        await fetch(`${API_URL}/puntos`, {
            method: "GET",
        })
            .then((res) => res.json())
            .then((data) => {
                if (data.error) notify.show(data.body, "error");
                    ...
                    setValorPuntos(data.body[0].valor);
                }
            })
                    ...
                    if (data.error) notify.show(data.body, "error");
                        ...
                        if (data.body !== null) {
                            ...
                            if (productFilter.length > 0) {
                                ...
                                setStockDisponible(
                                    `${pro.producto.name} stock disponibles: ${pro.producto.stock - 1}`,
                                );
                                setPuntosAcumulados(
                                    pro.producto.descuento > 0
                                        ? floor10(
    ...
    const arrayHandlerProduct = (pro) => {
        ...
        for (let i = 0; i < productFilter.length; i++) {
            if (productFilter[i].code === pro.producto.code) {
                ...
                setStockDisponible(
                    `${aux[i].name} stock disponibles:  ${aux[i].stock - aux[i].cantidad}`,
                );
            }
        }
        if (!a) {
    ...
    const setCantidad = (index, can) => {
        ...
        setProductFilter([]);
        setStockDisponible(
            `${productFilter[index].name} stock disponibles:  ${productFilter[index].stock - productFilter[index].cantidad}`,
        );
        setProductFilter(aux);
        handlerTotal();
    };
            ...
            .then((data) => {
             ...
             Puntos acumulados: ${data.body.persons[0].puntos}`;
                        setIdCliente({
                            ...
                            puntos: data.body.persons[0].puntos,
                        });
                    } else {
                        setIdCliente(false);
    ...
    function confirmarVenta() {
        ...
        var venta = {
            ...
            descuentoPuntos: puntosCanjeados > 0 ? puntosCanjeados : 0,
        };
        ...
        if (!idCliente) {
            ...
            fetch(`${API_URL}/venta`, {
                ...
                .then((resObj) => {
                    ...
                    setIdCliente(false);
                    setStockDisponible(0);
                    if (canjearPuntos && puntosAcumulados > 0 && puntosCanjeados > 0) {
                        fetch(`${API_URL}/person/${idCliente.id}`, {
                            method: "PATCH",
                            body: JSON.stringify({
                                puntos: puntosCanjeados * -1,
                            }),
                            headers: {
                                Authorization: `Bearer ${token}`,
                                "Content-Type": "application/json",
    ...
    async function handlerSearch() {
        if (event.target.value && token && event.target.value.length > 0) {
            const u = await fetch(
                `${API_URL}/inventario/buscar/termino?termino=${event.target.value}&idSucursal=${getAdmSucursal}`,
                {
                    method: "GET",
                    headers: {
                        Authorization: "Bearer " + token,
    ...
    async function handlerCanjearPuntos() {
        if (event.target.value && event.target.value.length > 0 && idCliente) {
            console.log(event.target.value);
            if (parseInt(event.target.value) > idCliente.puntos) {
                notify.show(
                    "El cliente no tiene esa cantidad de puntos para canjear",
                    "error",
                );
            } else {
                setPuntosCanjeados(parseInt(event.target.value));
            }
        } else {
            notify.show("Por favor ingrese a un cliente", "warning");
        }
    }
    return (
        <>
                                                                        ...
                                                                        (datos, index) => (
                                                                            <ul key={index} className="list-group">
                                                                                <li
                                                                                    ...
                                                                                    onClick={() => handlerClick(datos.producto[0], datos.producto[0].stock = datos.stockTotal)}>
                                                                                    <a>
                                                                                        <strong>{datos.producto[0].name}</strong>
                                                                                    </a>
                                                                                </li>
                                                                            </ul>
                                                                        ),
                                        ...
                                        <div className="row">
                                            ...
                                            <div className="col-lg-6">
                                                <div className="custom-control custom-checkbox">
                                                    ...
                                                    <label
                                                        ...
                                                        Cangear Puntos del cliente por descuento en la
                                                        compra
                                                    </label>
                                                </div>
                                                {canjearPuntos ? (
                                            ...
                                            {canjearPuntos ? (
                                                <div className="col-lg-6">
                                                    <div className="form-group mb-3">
                                                        <label className="form-label">
                                                            Cada punto tiene un valor de: {valorPuntos} Bs
                                                        </label>
                                                        <label
                                                            className="form-label"
                                                            style={{ color: "green", fontSize: "20px" }}>
                                                            Se descontara del total a pagar:{" "}
                                                            {puntosCanjeados > 0
                                                                ? (puntosCanjeados * valorPuntos).toFixed(2)
                                                                : 0}{" "}
                                                            Bs
                                                        </label>
                                                        <label
                                                            ...
                                                            style={{ color: "green", fontSize: "20px" }}>
                                                            Total a pagar:{" "}
                                                            {(total - puntosCanjeados * valorPuntos).toFixed(
                                                                2,
                                                            )}{" "}
                                                            Bs
                                                        </label>
