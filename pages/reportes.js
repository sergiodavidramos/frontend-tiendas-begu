...
const Reportes = () => {
    ...
    async function reportePocoStockProductos(idSucursal, token, cantidad = 5) {
        try {
            const datos = await fetch(
    ...
    async function reporteProductosVencimiento(idSucursal, token, dias = 15) {
        try {
            const datos = await fetch(
        ...
        try {
            if (restaurar === false) {
                ...
                if (traslados.error)
                    ...
                    a.href = url;
                    a.download = `Backup/${fecha.getDate() +
                        "-" +
                        (fecha.getMonth() + 1) +
                        "-" +
                        fecha.getFullYear()
                ...
                const datos = await fetch(`${API_URL}/backup?restaurar=${restaurar}`, {
                    method: "GET",
                    headers: {
                        Authorization: `Bearer ${token}`,
                        "Content-Type": "application/json",
                                            ...
                                            <div className="news-content-right pd-20">
                                                <div className="form-group">
                                                    ...
                                                    <select
                                                        ...
                                                        <option value="1">
                                                            Reporte para obtener los productos mas vendidos
                                                        </option>
                                                        <option value="2">
                                                            Reporte para obtener todos los Ingresos
                                                        </option>
                                                        <option value="3">
                                                            Reporte para obtener todos los Egresos o compras
                                                        </option>
                                                        <option value="4">
                                                ...
                                                {reporteSeleccionado === "5" && (
                                                    <div className="form-group">
                                                        <label className="form-label">
                                                            Productos con fecha de vencimiento menor que:*
                                                        </label>
                                                        <input
                                                            type="Number"
                                                            className="form-control datepicker-here"
