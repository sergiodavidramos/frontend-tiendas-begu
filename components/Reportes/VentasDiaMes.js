...
const VentasDiaMes = ({ adminSucursal, token }) => {
    ...
    const [capitalSucursal, setCapitalSucursal] = useState([]);
    ...
    useEffect(() => {
        if (adminSucursal && token) {
            ...
            capitalTotal(adminSucursal, token)
        }
    }, [adminSucursal, token]);
    async function getVentasDia(idSucursal, token) {
        try {
            ...
            console.log("Error al obtener la informacion de ventas del dia", error);
        }
    }
    ...
    async function capitalTotal(idSucursal, token) {
        try {
            const datos = await fetch(
                `${API_URL}/inventario/reporte/capital-inventario/${idSucursal}`,
                {
                    method: "GET",
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            const capitalTotal = await datos.json();
            if (capitalTotal.error) notify.show("Error en la peticion mes", "error");
            else {
                // console.log(capitalTotal.body)
                setCapitalSucursal(capitalTotal.body);
            }
        }
        catch (error) {
            console.log("Error al obtener la informacion del capital", error)
        }
    ...
    return (
        <>
            ...
            <div className="card card-static-2 mb-30">
                <div className="card-title-2">
                    <h4>Catital Total del negocio</h4>
                </div>
                <div className="card-body-table">
                    <div className="table-responsive">
                        <table className="table ucp-table table-hover" id="tablaVentasDia">
                            <thead>
                                <tr>
                                    <th>Cantidad de productos Registrados</th>
                                    <th>Total capital Bs.</th>
                                </tr>
                            </thead>
                            <tbody>
                                {capitalSucursal.length > 0 &&
                                    capitalSucursal.map((cap, index) => {
                                        return (
                                            <tr key={index}>
                                                <td>
                                                    {cap.cantidadProductos}
                                                </td>
                                                <td>
                                                    {cap.capitalTotal}
                                                </td>
            ...
            <div className="card card-static-2 mb-30">
                ...
                <div className="card-body-table">
                    <div className="table-responsive">
                        <table className="table ucp-table table-hover" id="tablaVentasMes">
                            <thead>
                                <tr>
                                    <th>Año</th>
                                    <th>Mes</th>
                    ...
                    <ul>
                        <li>
                            <ReactHTMLTableToExcel
                                ...
                                filename={`Venta de cada mes del año ${new Date().getFullYear}`}
                                sheet="pagina 1"
                                buttonText="Exportar a Excel"
                            />
                        </li>
